...
export const State = {
  ...
  currentSession() { return this.data.sessions[0] || this.newSession(); }
};
State.load();
...
export function importData(file) {
  return new Promise((resolve, reject) => {
    ...
    r.onload = () => {
      try { const json = JSON.parse(r.result); State.data = json; State.save(); resolve(json); }
      catch (e) { reject(e); }
    };
    r.onerror = reject;
    r.readAsText(file);
...
export function setThemeFromToggle() {
  ...
  State.data.settings.theme = next; State.save(); applyTheme(next);
                                    }
